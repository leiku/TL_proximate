...
for(i in 1:6){
  ...
  for(j in 1:length(res1)){
    ...
    y.spat[j]<-1.047*res2[8,1]+0.008/res2[7,1]-0.414
    y.temp[j]<-1.983*res2[8,1]+0.586*res2[7,1]-1.281
  }
  X.spat[[i]]<-x.spat
  X.temp[[i]]<-x.temp
  Y.spat[[i]]<-y.spat
...
colnames(ans)<-c("r.spat","p.spat","r.temp","p.temp", "error.spat", "error.temp")
for(i in 1:7){
  if(i==1){
    x<-unlist(X.spat)
    y<-unlist(Y.spat)
...
write.csv(ans,"res_empirical_predict.csv",row.names = T)


# plot
cols<-c("red","gold3","green4","cyan3","blue","purple")
tiff(paste0("Fig_empirical_predict.tif"),width=7, height=3.5, units="in",compression="zip",res=300)
